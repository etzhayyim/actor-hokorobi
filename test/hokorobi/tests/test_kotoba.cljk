...
(defn- tmp [] (str (System/getProperty "java.io.tmpdir") "/hokorobi-ledger-test-" (gensym) ".edn"))
(defn- d1 [] [(k/add "inst:gsib-bank" ":organism/kind" ":institution")
              (k/add "inst:gsib-bank" ":inst/sii" ":g-sib")])
(defn- d2 [] [(k/add "en.risk:contagion.stresses.inst:gsib-bank" ":en/kind" ":stresses")
              (k/add "en.risk:contagion.stresses.inst:gsib-bank" ":en/risk-load" 0.7)])
...
(deftest float-risk-load-roundtrips
  (let [p (tmp)]
    (try
      (k/append-tx (k/make-tx (d2) "t1" "as1" "") p)
      (is (= (d2) (get (first (k/read-log p)) ":tx/datoms")) "float :en/risk-load round-trips")
      (finally (io/delete-file p true)))))
  ...
  (let [p (tmp)]
    (try
      (let [c1 (k/append-tx (k/make-tx (d1) "t1" "as1" "") p)]
        ...
        (let [corrupted (str/replace (slurp p) ":stresses" ":buffers")]
          (spit p corrupted)
...
;; ── heartbeat (autorun) over the REAL committed finrisk seed ─────────────────

(deftest beat-ground-only-and-real-seed
  (let [ds (auto/ground-datoms)]
    (is (seq ds) "ground datoms are produced from the real committed finrisk seed")
    (is (every? #(= ":db/add" (first %)) ds) "every datom is an :add (EAVT op)")
    (is (some (fn [[_ _ a]] (= ":en/from" a)) ds) "systemic-risk 縁 are persisted (graph not just nodes)")
    (is (not-any? (fn [[_ _ a]] (str/starts-with? (str a) ":bond/")) ds)
        "GROUND only — derived :bond/* readouts not persisted (N1/G2)")))

(deftest beat-no-trading-signal
  ;; G1: a RESILIENCE map, NEVER a trading/market signal — no price/trade/order/position datoms
  (let [ds (auto/ground-datoms)]
    (is (not-any? (fn [[_ _ a]] (let [s (str a)]
                                  (or (str/includes? s "price") (str/includes? s "trade")
                                      (str/includes? s "order") (str/includes? s "position")
                                      (str/includes? s "signal")))) ds)
        "no trading/market-signal datoms in the ledger (G1, never-trades)")))
...
#?(:clj
   (when (= *file* (System/getProperty "babashka.file"))
     (let [{:keys [fail error]} (run-tests 'hokorobi.tests.test-kotoba)]
