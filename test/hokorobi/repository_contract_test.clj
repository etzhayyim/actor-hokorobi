...
(deftest canonical-repository-shape
  (doseq [path ["manifest.edn" "identity.edn" "dependencies.edn"
                ...
                "schema/finrisk-ontology.kotoba.edn"]]
  ...
  (is (empty? (filter #(or (.endsWith (.getName %) ".bpmn")
                            (.endsWith (.getName %) ".jsonld"))
                      (remove #(str/includes? (.getPath %) (str "wire" java.io.File/separator))
                              (file-seq (io/file ".")))))))
